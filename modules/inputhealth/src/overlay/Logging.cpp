...
// Drop any overlay_log.*.txt older than 24 hours.
void ClearOldLogs(const std::wstring& dir)
{
	std::wstring search = dir + L"\\overlay_log.*.txt";
...
// %LocalAppDataLow%\OpenVR-Pair\Logs\overlay_log.<date>T<time>.txt
std::wstring BuildLogPath()
{
	...
	std::wstring path = dir + L"\\overlay_log." + date.data() + L"T" + time.data() + L".txt";
	return path;
}
